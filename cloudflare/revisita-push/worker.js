...
const APP_ID = 'revisita';
const APP_PATH = '/Revisita/';
const DEFAULT_ALLOWED_ORIGIN = 'https://davidfontenelle80-cloud.github.io';
const DEFAULT_TTL_SECONDS = 60 * 60 * 24 * 366;
...
async function handleSubscribe(request, env) {
  ...
  return json({ ok: true, id, subscriptionId: id, token: record.token }, 200, headers);
}

function authorized(request, record) {
  return record && request.headers.get('authorization') === `Bearer ${record.token}`;
}
...
async function handleUpsertReminder(request, env) {
  ...
  if (!authorized(request, subscription)) return json({ ok: false, error: 'Unauthorized.' }, 403, headers);
  const fireTime = Date.parse(fireAt);
  if(sourceType !== 'revisita' || !/^[\w-]{1,80}$/.test(sourceId) || !Number.isFinite(fireTime) || fireTime < Date.now() - 60000 || fireTime > Date.now() + 365 * 86400000) {
    return json({ ok: false, error: 'Invalid reminder or time beyond one year.' }, 400, headers);
  }
  ...
  const record = {
    ...
    sourceId,
    title: 'Revisita',
    body: String(data.body || '').slice(0, 120),
    fireAt: new Date(fireTime).toISOString(),
...
async function handleTestPush(request, env) {
  ...
  if (!authorized(request, subRecord)) return json({ ok: false, error: 'Unauthorized.' }, 403, headers);

  const payload = {
    title: data.title || 'Revisita',
