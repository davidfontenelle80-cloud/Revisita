...
function persist(announce=true){try{saveState(state);setStatus(navigator.onLine?'Guardar':'Sin conexión',navigator.onLine?'success':'warn',announce);}catch(e){setStatus('No se pudo guardar','danger');showError('Error al guardar',e.message||String(e));}}
...
function updateOnline(){setStatus(navigator.onLine?'Guardar':'Sin conexión',navigator.onLine?'success':'warn');}
